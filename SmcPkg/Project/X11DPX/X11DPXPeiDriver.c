 ...
 * --------------------------------------------------*/
//HSX_PER_LANE_EPARAM_LINK_INFO  X11DPXPerLanesEparamTable[] = {
//};
//UINT32 X11DPXPerLanesEparamTableSize = sizeof (X11DPXPerLanesEparamTable);
...
ALL_LANES_EPARAM_LINK_INFO  X11DPXAllLanesEparamTable[] = {
...
UINT32 X11DPXAllLanesEparamTableSize = sizeof (X11DPXAllLanesEparamTable);
...
)
{
    ...
    *SizeOfTable = X11DPXAllLanesEparamTableSize;
    *Ptr = (ALL_LANES_EPARAM_LINK_INFO *) &X11DPXAllLanesEparamTable;
...
)
{
    ...
    GpioGetInputValue(CPU1_SLOT2_16_Pin, &Slot2Pin);
    GpioGetInputValue(CPU2_SLOT6_16_Pin, &Slot6Pin);
    DEBUG((-1, "Slot2Pin = %x\n", Slot2Pin));
    DEBUG((-1, "Slot6Pin = %x\n", Slot6Pin )); 
    //
    // First CPU
    ...
    if(SetupData->ConfigIOU0[0] == 0xff ){
      if( Slot2Pin == 0){  
        ...
        SetupData->SLOTPSP[1] = 2;
        SetupData->SLOTIMP[3] = 1;
        SetupData->SLOTPSP[3] = 1;
      } else {
    ...
    if(SetupData->ConfigMCP0[0] == 0xff) {
        if(PcdGet64(PcdSmcCPU0IIOBus) & BIT56) { //FPGA CPU
            ...
            SetupData->SLOTPSP[13] = 13;
            SetupData->SLOTIMP[15] = 1;
            SetupData->SLOTPSP[15] = 15;
        } else {
    ...
    if(SetupData->ConfigIOU0[1] == 0xff ){
      if( Slot6Pin == 0){  
        SetupData->ConfigIOU0[1] = IIO_BIFURCATE_xxx8xxx8;
        SetupData->SLOTIMP[22] = 1;
        SetupData->SLOTPSP[22] = 6;
        SetupData->SLOTIMP[24] = 1;
        SetupData->SLOTPSP[24] = 5;
      } else {
    ...
    if(SetupData->ConfigIOU1[1] == 0xff) {
        SetupData->ConfigIOU1[1] = IIO_BIFURCATE_xxx8xxx8;
        SetupData->SLOTIMP[26] = 1;
        SetupData->SLOTPSP[26] = 10;
        SetupData->SLOTIMP[28] = 1;
        SetupData->SLOTPSP[28] = 9;
    }

    if(SetupData->ConfigIOU2[1] == 0xff) {
        SetupData->ConfigIOU2[1] = IIO_BIFURCATE_xxxxxx16;
        SetupData->SLOTIMP[30] = 1;
        SetupData->SLOTPSP[30] = 8;
    }
...
//----------------------------------------------------------------------------
//<AMI_PHDR_START>
EFI_STATUS EFIAPI X11DPXPeiDriverInit(
