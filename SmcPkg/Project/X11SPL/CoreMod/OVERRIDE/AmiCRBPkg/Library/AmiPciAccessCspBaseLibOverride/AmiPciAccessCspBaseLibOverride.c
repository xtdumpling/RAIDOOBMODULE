...
**/
//*************************************************************************
#include <AmiDxeLib.h> //SMCPKG_SUPPORT+
//-------------------------------------------------------------------------
// Include Files
//-------------------------------------------------------------------------
#include <Token.h>
                ...
                if (IohOutofResInfo.MmioRequested_Stack_Level[Socket][StackIndex][FailedMMioIndex] > 0 ) 
                {
                    ...
                    if (FistbitSetPoistioninTempBaseAddress >= FistbitSetPoistioninMaxGra)
                    {
                        ...
                        else
                        {
                            ...
                            if ((StackIndex == (NumberofStacksPerCpu -1)) && (TempBaseAddressBeforeAlignemnt < IohOutofResInfo.MmioLimit_Stack_Level[Socket][StackIndex]) && AfterStackAdjust_SocketMMIO_AdjustNeeded == FALSE && SocketLevelMMIoAdjustmentNeeded == FALSE) 
                            {
    ...
    for (StackIndex = 0; StackIndex < MAX_IIO_STACK; StackIndex++) 
    {
      ...
      if (OutOfResrcFailure == MMIOFAILURE )
      {
        if (IohOutofResInfo.MmmioOutofResrcFlag_Stack_Level [SocketNumber][StackIndex] &&
            RequiredMMIOIoRatio.MMIO_Stack_Level[SocketNumber][StackIndex] != AutoAdjustMmioIoVariable.MmiolRatio_Stack_Level[SocketNumber][StackIndex]) 
              return FALSE;
      }
      ...
      if (OutOfResrcFailure == IOFAILURE )
      {
         if (IohOutofResInfo.IoOutofResrcFlag_Stack_Level [SocketNumber][StackIndex] &&
             RequiredMMIOIoRatio.IO_Stack_Level[SocketNumber][StackIndex] != AutoAdjustMmioIoVariable.IoRatio_Stack_Level[SocketNumber][StackIndex])
             return FALSE;
      }
    }
    return TRUE;
	...
	if ((StackLevelAdjustmentNeeded == TRUE))
	{
		for (FailedSocketNumber = 0 ; FailedSocketNumber < NbGetMaximumIohSockets() ; FailedSocketNumber++)
		{
			if (Stack_MMIO_Failed_Socket [FailedSocketNumber] != 0xFF)
			{
			  ...
			  if (StackAdjustedDuringSocketAdjustment (FailedSocketNumber,MMIOFAILURE) == TRUE)
			  {
...
IO_Check:			
			...
			{
			  if (StackAdjustedDuringSocketAdjustment (FailedSocketNumber,IOFAILURE) == TRUE )
			  {
	...
	//          in Next boot.
...
#if SMCPKG_SUPPORT
		{
		  ...
		  if (GetBootMode() == BOOT_WITH_DEFAULT_SETTINGS)
		  {
			  ...
			  Value = IoRead8 (R_IOPORT_CMOS_IDX_DIAGNOSTIC_STATUS);
			  IoWrite8 (R_IOPORT_CMOS_STANDARD_DATA, Value | 0x0C0);
		  }
		}
