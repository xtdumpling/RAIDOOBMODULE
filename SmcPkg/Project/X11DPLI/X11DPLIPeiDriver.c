...
)
{
    ...
    {
    	SetupData->ConfigIOU0[0] = IIO_BIFURCATE_xxx8xxx8; // #1A:Slot6/x8, #1B:Slot4/x8
    }

    // CPU0-#2
    if(SetupData->ConfigIOU1[0] == 0xff)
    {
    	SetupData->ConfigIOU1[0] = IIO_BIFURCATE_xxxxxx16; // #2:Slot5/x16
    }

    // CPU0-#3
    if(SetupData->ConfigIOU2[0] == 0xff)
    {
    	SetupData->ConfigIOU2[0] = IIO_BIFURCATE_xxx8x4x4; // #3A:UpLink/x4, #3B:Slot1/x4, #3C:Slot2/x8
    }

    // CPU1-#3
    if(SetupData->ConfigIOU2[1] == 0xff)
    {
    	SetupData->ConfigIOU2[1] = IIO_BIFURCATE_xxxxxx16; // #3:Slot3/x16
    }
    ...
    if(!EFI_ERROR(Status)){
        ...
        Status = ReadOnlyVariable->GetVariable(ReadOnlyVariable, L"Setup", &SetupGuid, NULL, &VarSize, &GbESetup);
        if(!EFI_ERROR(Status)) {
            if(GbESetup.SmcOnboardLan == 0) {
            	SetupData->PciePortDisable[9] = 0;  // 1000BASE-T LAN map to CPU PCIe port 3A
                GpioSetOutputValue(SMC_LAN_DISABLE_GPIO, 1); // Disable Lan Power
            }
        }
    }
#endif
...
//----------------------------------------------------------------------------
//<AMI_PHDR_START>
EFI_STATUS EFIAPI X11DPLIPeiDriverInit(
