...
)
{
    ...
    DEBUG((-1, "X11DPUHideSetupItem Entry.\n"));

    gBS->CloseEvent(Event);

    Status = gBS->LocateProtocol (&gSetupModifyProtocolGuid, NULL, &SetupModifyProtocol);
    if (EFI_ERROR (Status)) {
        DEBUG((-1, "ERROR : X11DPUHideSetupItem - Can't locate gSetupModifyProtocolGuid.\n"));
        return;
    }
    ...
    DEBUG((-1, "X11DPUHideSetupItem end.\n"));
}
