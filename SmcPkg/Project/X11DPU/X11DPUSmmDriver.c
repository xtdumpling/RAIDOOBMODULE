...
//
//  Rev. 1.02
//    Bug Fix:  Fixed SMM driver load failed.
//    Reason:   
//    Auditor:  Jacker Yeh & Jason Hsu
//    Date:     Jul/28/2017
//
//  Rev. 1.01
...
//----------------------------------------------------------------------------
//
// Procedure:   X11DPUPowerButtonSmiHandler
//
    ...
    IN EFI_SYSTEM_TABLE     *SystemTable )
{
    ...
    Status = gSmst->SmmLocateProtocol (&mEfiSmmSmbusProtocolGuid, NULL, &SmbusHc);
    if (EFI_ERROR (Status)) {
	//DEBUG((EFI_D_INFO, "X11DPU : can't locate SmmSmbusProtocolGuid.\n"));
        return Status;
    }

    Status = gSmst->SmmInstallProtocolInterface (
                      &Handle,
                      &mEfiSmcSmmSmbusProtocolGuid,
                      EFI_NATIVE_INTERFACE,
                      SmbusHc
                      );
    if (EFI_ERROR (Status)) {
        //DEBUG((EFI_D_INFO, "X11DPU : can't install SmcSmmSmbusProtocolGuid.\n"));
        return Status;
    }
...
EFI_STATUS X11DPUSmmDriverInit (
    IN EFI_HANDLE           ImageHandle,
    IN EFI_SYSTEM_TABLE     *SystemTable )
{
