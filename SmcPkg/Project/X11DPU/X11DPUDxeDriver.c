...
)
{
    ...
    *(UINT8*)MmPciAddress(0, (UINT8)(BDF >> 8), (UINT8)((BDF >> 3) & 0x1f), (UINT8)(BDF & 0x07), 0x04) |= 0x06;
    ...
    if(Is710) {
        ...
        if (Index == 1)
            SmcGetMacFrVpd (BDF, 1, TempBuffer + i + 1);
        else
...
)
{
    ...
    DEBUG((EFI_D_INFO, "X11DPUReadyToBootCallBack Entry.\n"));
...
)
{
    ...
    UINTN VariableSize = sizeof(SOCKET_MEMORY_CONFIGURATION);
    ...
    //Check if MemErrorLedOff set, clear it(done in PEI, but variable need modify in DXE)
    if((ReadCMOS(CMOS_MEM_ERR_FLAG_0) == 0x55) && (ReadCMOS(CMOS_MEM_ERR_FLAG_1) == 0xAA)){    
        Status = pRS->GetVariable(SOCKET_MEMORY_CONFIGURATION_NAME,
                                  &gSmcEfiSocketMemoryVariableGuid,
                                  NULL,
                                  &VariableSize,
                                  &gSocketMemoryConfiguration
                                 );
    
    
        gSocketMemoryConfiguration.MemErrorLedOff = 0;
    
        Status = pRS->SetVariable(L"SocketMemoryConfig",
    				    &gSmcEfiSocketMemoryVariableGuid,
