...
)
{
    ...
    UINT8 LanBus;
    UINT32 ReturnVal0 = 0, ReturnVal1 = 0, ReturnVal2 = 0, ReturnVal3 = 0, SIOM_ID = 0, SXB3_ID = 0, SXB4_ID = 0;

    gBS->CloseEvent(Event);
    
    Status = gBS->LocateProtocol (&gSetupModifyProtocolGuid, NULL, &SetupModifyProtocol);
    if (EFI_ERROR (Status)) {
        DEBUG((-1, "ERROR : X11DPTPSHideSetupItem - Can't locate gSetupModifyProtocolGuid.\n"));
        return;
    }
//Riser
...
//Read Data
    ...
    DEBUG((EFI_D_INFO, "X11DPTPSHideSetupItem BusNum = %x, LanBus = %x, Data32 = %x UpdatedLAN1BDF = %x End.\n",BusNum, LanBus, Data32, (UpdatedLAN1BDF >> 8)));
