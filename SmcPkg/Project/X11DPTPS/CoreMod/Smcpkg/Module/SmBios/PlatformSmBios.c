...
//Get SSVID and SSID
    ...
    if((ClassCode == 0x01) && (SubClassCode == 0x08))
...
//AssetInfo area
        ...
        *(UINT32*)(AssetInfo_H->VID_DID) = *(UINT32*)MmPciAddress(0, *Bus, *Dev, *Fun, 0);
...
)
{
    ...
    UINT32	TempBDF;
    UINT32  SIOMBDF, ReturnVal0 = 0, ReturnVal1 = 0, ReturnVal2 = 0, ReturnVal3 = 0, SIOM_ID = 0;	
...
//
    ...
    SIOMBDF = SmcBDFTransfer(0x00110000);	//PcdSmcSLOT5BDF, real values depends on project
    // AOC-MTG-I4T/I2T   : 1100b
    // AOC-MHIBF-2QG/1QG : 1010b
    // AOC-MHIBF-m2Q2G
    // AOC-MHIBF-m1Q2G
    
    if ((Bus == (SIOMBDF>>8)) && (Dev == ((SIOMBDF&0xFF)>>3)) && (Fun == ((SIOMBDF&0x07))))	//SIOM
    {
      ...
      if((SIOM_ID == 0x0C) || (SIOM_ID == 0x0A))
      {
        if(*(UINT16*)MmPciAddress(0, (*(UINT16*)MmPciAddress(0, Bus, Dev, Fun, 0x18)), 0, 0, 0) == 0xFFFF)
        {
           SIOMBDF = SmcBDFTransfer(0x00110010);	//Port 3A
           Dev = (SIOMBDF >> 3)& 0x1F;
        }
      }
    }
...
// Update slot CurrentUsage
    if(*(UINT32*)MmPciAddress(0, Bus, 0, 0, 0) == 0xFFFFFFFF)
...
//  SmBiosType40Create(UpdateHandle, &Bus, &Dev, &Fun, &BusE, 0x0000);	//SIOM already create in type 9
