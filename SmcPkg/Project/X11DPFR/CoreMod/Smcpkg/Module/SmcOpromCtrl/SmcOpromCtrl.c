...
)
{
    ...
    DEBUG((-1,"Entry Legacy ROM Device in Bus:%02x Dev:%02x Fun:%02x\n", PciBus, PciDev, PciFun));
#if Multi_VGA == 1
...
//X11DRT-PS, Only treat i350 SIOM card as onboard LAN.	
LAN_ID = *(UINT32*)MmPciAddress(0, (UINT8)PciBus, (UINT8)PciDev, (UINT8)PciFun, 0);
if((LAN_ID == 0x15218086) || (LAN_ID == 0x15338086)){
   DEBUG((-1,"Legacy ROM Device in Bus:%02x Dev:%02x Fun:%02x, DeviceID = %x \n", PciBus, PciDev, PciFun, LAN_ID = *(UINT32*)MmPciAddress(0, (UINT8)PciBus, (UINT8)PciDev, (UINT8)PciFun, 0)));
...
DEBUG((-1,"SLOT Legacy ROM Device in Bus:%02x Dev:%02x Fun:%02x\n", PciBus, PciDev, PciFun));
	...
	if(TempBDF && (TempBDF != 0x8888)){
	    ...
	    DEBUG((-1,"SLOT TempBusMax: %02x TempBusMin: %02x Enable: %02x\n", TempBusMax, TempBusMin, SmcSlotOpromEnable_Array[i]));
...
//    		TempFun %= PcdGet8(PcdSmcMultiFuncLanChip2);
	...
	if(TempBDF && (TempBDF != 0x8888)){
	    ...
	    DEBUG((-1,"UEFI TempBusMax: %02x TempBusMin: %02x Enable: %02x\n", TempBusMax, TempBusMin, SmcSlotOpromEnable_Array[i]));
	    if(i < (sizeof(PciDevNumList)/sizeof(UINT8)))
