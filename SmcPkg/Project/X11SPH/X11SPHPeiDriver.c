...
//
//  Rev. 1.01
//    Bug Fix:  When disables Onboard LAN device, disables LAN Phy by asserting GPP_C8 and Upperlink 2.
//    Reason:   
//    Auditor:  William Hsu
//    Date:     Jan/12/2017
//
//  Rev. 1.00
//    Bug Fix:  Correct slot setting.
//    Reason:   n/a
//    Auditor:  William Hsu
//    Date:     Dec/30/2016
//
 ...
 * --------------------------------------------------*/
//HSX_PER_LANE_EPARAM_LINK_INFO  X11SPHPerLanesEparamTable[] = {
//};
//UINT32 X11SPHPerLanesEparamTableSize = sizeof (X11SPHPerLanesEparamTable);
...
)
{
    ...
    if(SetupData->ConfigIOU0[0] == 0xff){
        SetupData->ConfigIOU0[0] = IIO_BIFURCATE_xxx8x4x4;	//SAS x8/Oculink1 x4/Oculink2 x4
        SetupData->SLOTIMP[1] = 1;	//NVME
        SetupData->SLOTPSP[1] = 1;   
        SetupData->SLOTIMP[2] = 1;  //NVME
        SetupData->SLOTPSP[2] = 2;
        SetupData->SLOTIMP[3] = 1;  //LSI SAS 
        ...
        OnboardNVMe |= 0x0000000000000005;
        PcdSet64(PcdSmcAOCLocation, OnboardNVMe);
#endif
    }

    if(SetupData->ConfigIOU1[0] == 0xff){
        SetupData->ConfigIOU1[0] = IIO_BIFURCATE_xxx8xxx8;	//slot3 x8/ Uplink x8  
        SetupData->SLOTIMP[5] = 1;
        SetupData->SLOTPSP[5] = 5;
        SetupData->SLOTIMP[7] = 1;
	...
	if(temp){
	    SetupData->ConfigIOU2[0] = IIO_BIFURCATE_xxxxxx16;	//slot 6 x16
	    SetupData->SLOTIMP[9] = 1;
            SetupData->SLOTPSP[9] = 9;
	}
	else {
            SetupData->ConfigIOU2[0] = IIO_BIFURCATE_xxx8xxx8;	//slot 5 x8 / slot 6 x8
            SetupData->SLOTIMP[9] = 1;
            SetupData->SLOTPSP[9] = 9;
	...
	mSmcAOCPeiProtocol->SmcAOCPeiSetNVMeMode(PeiServices, 0x0d, 0, 0, NULL, 0); // BMC SMBUS port (7-1) x 2 + 1 = 0xd
    }
#endif
#if ONBOARD_LAN_DISABLE_BY_SETUP_ITEM
    Status = (*PeiServices)->LocatePpi(
	...
	Status = ReadOnlyVariable->GetVariable(
        ...
        if(!EFI_ERROR(Status)){
           if(Setup10GbE.SmcOnboardLan == 0){
               SetupData->PciePortDisable[5] = 0;    //10Gbe LAN map to CPU PCIe port 2A
               SetupData->PciePortDisable[6] = 0;    //10Gbe LAN map to CPU PCIe port 2B
...
//----------------------------------------------------------------------------
//<AMI_PHDR_START>
EFI_STATUS EFIAPI X11SPHPeiDriverInit(
