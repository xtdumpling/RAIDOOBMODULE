...
@if %SGI_B_BOARD% == 0 @set BIOS_NAME=%prj%_SA_T%TODAY4%%NOW3%.bin
@if %SGI_B_BOARD% == 1 @set BIOS_NAME=%prj%_SB_T%TODAY4%%NOW3%.bin
...
@REM # SiEn
spsFITC.exe -b -f %prj%_SiEn.xml -bios %WORKSPACE%\%OUTPUT_DIR%\FV\AMIROM.fd -opr spsOperational.bin -rcv spsRecovery.bin -o %WORKSPACE%\%BIOS_NAME%
@REM # NM
@REM # spsFITC.exe -b -f %prj%.xml -bios %WORKSPACE%\%OUTPUT_DIR%\FV\AMIROM.fd -opr spsOperational.bin -rcv spsRecovery.bin -o %WORKSPACE%\%BIOS_NAME%
