...
#include <PCI.h>
#endif
#include <Library/GpioLib.h> //X11SPM
...
//X11SPM >>>
    		// Because 88E1512's PCI Function is 2 and 3, add TempFun with 2
            if(SkuId==0) {
                if(i<2)
                    TempFun += 2;
                else
...
//X11SPM >>>
void X11SPMInitSkuId()
{
    UINT8 index;
    UINT32  X11SPM_SKU_GPIO[] = SKU_TBL_GPIO_Tbl, temp;
