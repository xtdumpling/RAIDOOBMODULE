...
void X11SpmUpdateSmBios()
{
    ...
    AMI_SMBIOS_PROTOCOL *SmbiosProtocol;
    UINT8       index, temp_ID;
    UINT32      temp, X11SPM_SKU_GPIO[] = SKU_TBL_GPIO_Tbl;
    ...
    // X11SPM-T4F
    if(temp_ID!=3) {
        // Delete LAN X722 #3 and LAN X722 #4 from SMBIOS Type 41
        ...
        if(!EFI_ERROR(Status)) {
            ...
            SmbiosProtocol->SmbiosDeleteStructure(Handle);   // X722 #3
            SmbiosProtocol->SmbiosDeleteStructure(Handle+1); // X722 #4
    ...
    // X11SPM-F
    if(temp_ID==0) {
        // Set the Device Function Number of the SMBIOS Type 41 for X722 #1
        Status = SmbiosProtocol->SmbiosReadStrucByType(41, 2, &TempBuffer, &TempBufferSize);
        Handle = ((SMBIOS_STRUCTURE_HEADER *)TempBuffer)->Handle;
        ((SMBIOS_ONBOARD_DEV_EXT_INFO*)TempBuffer)->DevFuncNumber = 2;
        Status = SmbiosProtocol->SmbiosWriteStructure(Handle, TempBuffer, TempBufferSize);
        gBS->FreePool(TempBuffer);

        // Set the Device Function Number of the SMBIOS Type 41 for X722 #2
        Status = SmbiosProtocol->SmbiosReadStrucByType(41, 3, &TempBuffer, &TempBufferSize);
        Handle = ((SMBIOS_STRUCTURE_HEADER *)TempBuffer)->Handle;
        ((SMBIOS_ONBOARD_DEV_EXT_INFO*)TempBuffer)->DevFuncNumber = 3;
        Status = SmbiosProtocol->SmbiosWriteStructure(Handle, TempBuffer, TempBufferSize);
        gBS->FreePool(TempBuffer);

        // Delete instance 3 and 4 from SMBIOS Type 8
        Status = SmbiosProtocol->SmbiosReadStrucByType(8, 3, &TempBuffer, &TempBufferSize);
        if(!EFI_ERROR(Status)) {
            Handle = ((SMBIOS_STRUCTURE_HEADER*)TempBuffer)->Handle;
            SmbiosProtocol->SmbiosDeleteStructure(Handle);
            SmbiosProtocol->SmbiosDeleteStructure(Handle+1);
            gBS->FreePool(TempBuffer);
        }
    }
    else if(temp_ID==1) { // X11SPM-TF
        // Delete instance 4 and 5 from SMBIOS Type 8
    ...
    else if(temp_ID==2) { // X11SPM-TPF
        // Delete instance 3 and 5 from SMBIOS Type 8
        Status = SmbiosProtocol->SmbiosReadStrucByType(8, 3, &TempBuffer, &TempBufferSize);
        if(!EFI_ERROR(Status)) {
            Handle = ((SMBIOS_STRUCTURE_HEADER*)TempBuffer)->Handle;
            SmbiosProtocol->SmbiosDeleteStructure(Handle);
            SmbiosProtocol->SmbiosDeleteStructure(Handle+2);
            gBS->FreePool(TempBuffer);
        }
    }
}
