...
//
//  Rev. 1.06
//      Bug Fixed:  Dynamic create type 41 for ultra IO.
//      Reason:     
//      Auditor:    Kasber Chen
//      Date:       Oct/14/2016
//
...
//
//  Rev. 1.03
//      Bug Fixed:  Fix all riser card slot oprom items can't show in setup.
//      Reason:     
//      Auditor:    Kasber Chen
//      Date:       Jul/04/2016
//
...
)
{
    ...
    for(i = 0; i < Board_Pcie_Table_Size; i++){
        ...
        case RC_SLOT:	//smbios type 9 information
            ...
            ((SMBIOS_SYSTEM_SLOTS_INFO*)TempBuffer)->BusNumber = (UINT8)(SmcBDFTransfer(Board_Pcie_Table[i]) >> 8);
            ((SMBIOS_SYSTEM_SLOTS_INFO*)TempBuffer)->DevFuncNumber = (UINT8)SmcBDFTransfer(Board_Pcie_Table[i]);
	    ...
	    gBS->SetMem(TempBuffer, 100, 0x00);				//clear buffer
            for(x = 0; x < (RC_Table[RC_Table_index].SLOT_NUM[i] & 0x0f); x++){
                ...
                ((SMBIOS_ONBOARD_DEV_EXT_INFO*)TempBuffer)->BusNumber = (UINT8)(SmcBDFTransfer(Board_Pcie_Table[i]) >> 8);
                ((SMBIOS_ONBOARD_DEV_EXT_INFO*)TempBuffer)->DevFuncNumber = (UINT8)SmcBDFTransfer(Board_Pcie_Table[i]);
...
)
{
    ...
    DEBUG((-1, "CreateAOMType9Smbios for X11DDW combine case.\n"));
    gBS->AllocatePool(EfiBootServicesData, 255, &TempBuffer);
    gBS->SetMem(TempBuffer, 255, 0x00);				//clear buffer
    ...
    ((SMBIOS_SYSTEM_SLOTS_INFO*)TempBuffer)->SlotDataBusWidth = 0x0B;	//8X
    ((SMBIOS_SYSTEM_SLOTS_INFO*)TempBuffer)->SlotLength = 0x03;	//LengthShort
    ((SMBIOS_SYSTEM_SLOTS_INFO*)TempBuffer)->SlotID = 0x0701;
    ((SMBIOS_SYSTEM_SLOTS_INFO*)TempBuffer)->BusNumber = (UINT8)(SmcBDFTransfer(0x00120000) >> 8);
    ((SMBIOS_SYSTEM_SLOTS_INFO*)TempBuffer)->DevFuncNumber = (UINT8)SmcBDFTransfer(0x00120000);
    Sprintf((UINT8*)&TempBuffer[0x11], "JAOM SLOT1");
    TempBufferSize = sizeof(SMBIOS_SYSTEM_SLOTS_INFO) + (UINT16)AsciiStrLen((UINT8*)&TempBuffer[0x11]) + 2;
    SmbiosProtocol->SmbiosAddStructure(TempBuffer, TempBufferSize);	//Add SMBIOS structure
