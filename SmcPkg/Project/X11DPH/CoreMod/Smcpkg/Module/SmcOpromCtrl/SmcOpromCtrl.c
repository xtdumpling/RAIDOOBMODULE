...
//
//  Rev. 1.15
//      Bug Fixed:  Fix OPROM control for Omni-Path UEFI driver.
//      Reason:     Miss to load the UEFI driver
//      Auditor:    Max Mu
//      Date:       Mar/18/2017
//
//  Rev. 1.14
...
)
{
    ...
    //
    // SKX-F Omni-Path integratd HFI device, EFI driver only
    //
#ifdef X11DPH_SUPPORT
    if( ((PciBus == 0xFF) || (PciBus == 0x7F))
        && (PciDev == 0x0) && (PciFun == 0x0) )
    {
        DID = *(UINT32*)MmPciAddress(0, (UINT8)PciBus, (UINT8)PciDev, (UINT8)PciFun, 0);
        DEBUG((-1,"DID : %04x\n", DID));
        if(DID == 0x24F18086)
            return EFI_SUCCESS;
    }
