call ..\SetSmcEnv.bat
set GBE_BIN_FILE=722PAD1A_PXE.BIN
@REM set GBE_BIN_FILE=722PAD10_PXE.BIN
...
spsFITC.exe -b -f %prj%.xml -bios %WORKSPACE%\%OUTPUT_DIR%\FV\AMIROM.fd -opr spsOperational.bin -rcv spsRecovery.bin -gbea %GBE_BIN_FILE% -o %WORKSPACE%\%BIOS_NAME% -flashsize1 %IFWI_FLASH_SIZE%
...
spsFITC.exe -b -f %prj%.xml -bios %WORKSPACE%\%OUTPUT_DIR%\FV\AMIROM.fd -opr spsOperational.bin -rcv spsRecovery.bin -gbea %GBE_BIN_FILE% -o %WORKSPACE%\%BIOS_NAME% -flashsize1 7
