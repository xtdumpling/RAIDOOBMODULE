...
{
    UINT32	X11SPI_GPIO[] = SLOT6_GPIO_Tbl, temp;
    UINT8	index, temp_ID = 0;
    EFI_STATUS Status = EFI_SUCCESS;
    SMC_SETUP_MODIFY_PROTOCOL *SetupModifyProtocol;

    DEBUG((-1, "X11SPIHideSetupItem entry.\n"));
    ...
    for(index = 0; index < 4; index++){
    	GpioGetInputValue(X11SPI_GPIO[index], &temp);
    	temp_ID |= temp << index;
    }

    if((temp_ID & 0x0f) == 0x0f)	//no any riser
	SetupModifyProtocol->Add2HideList("Setup", STR_RSC_RR1U_E16);	//ID = 0x0E

    DEBUG((-1, "X11SPIHideSetupItem end.\n"));
    return;
}
