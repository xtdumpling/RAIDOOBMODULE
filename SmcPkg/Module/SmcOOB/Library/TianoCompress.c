...
//
//#define NC    (UINT8_MA X + MAXMATCH + 2 - THRESHOLD)
#define CBIT  9
#define NP    (WNDBIT + 1)
#define PBIT  5
...
--*/
{
  ...
  pBS->AllocatePool (EfiBootServicesData, WNDSIZ * 2 + MAXMATCH, &mText);
  ...
  pBS->AllocatePool (EfiBootServicesData, (WNDSIZ + UINT8_MAX + 1) * sizeof (*mLevel), &mLevel);
  pBS->AllocatePool (EfiBootServicesData, (WNDSIZ + UINT8_MAX + 1) * sizeof (*mChildCount), &mChildCount);
  pBS->AllocatePool (EfiBootServicesData, (WNDSIZ + UINT8_MAX + 1) * sizeof (*mPosition), &mPosition);
  pBS->AllocatePool (EfiBootServicesData, WNDSIZ * 2 * sizeof (*mParent), &mParent);
  pBS->AllocatePool (EfiBootServicesData, WNDSIZ * 2 * sizeof (*mPrev), &mPrev);
  pBS->AllocatePool (EfiBootServicesData, (MAX_HASH_VAL + 1) * sizeof (*mNext), &mNext);

  mBufSiz     = BLKSIZ;
  //mBuf        = malloc (mBufSiz);
  pBS->AllocatePool (EfiBootServicesData, mBufSiz, &mBuf);
  while (mBuf == NULL) {
...
--*/
{
  ...
  if (mChildCount != NULL) {
    pBS->FreePool (mChildCount);
  }

  if (mPosition != NULL) {
    pBS->FreePool (mPosition);
  }

  if (mParent != NULL) {
    pBS->FreePool (mParent);
  }
...
--*/
{
  ...
  mPos++;
  if (mPos == WNDSIZ * 2) {
    MemCpy (&mText[0], &mText[WNDSIZ], WNDSIZ + MAXMATCH);
...
--*/
{
  ...
  // fprintf(stdout,"mRemainder: %d\n",mRemainder);
  mMatchLen   = 0;
  mPos        = WNDSIZ;
  InsertNode ();
  // fprintf(stdout,"mMatchLen: %d\n",mMatchLen);
...
--*/
{
  ...
  while (Number >= mBitCount) {
	  //// fprintf(stdout,"Number = %d, mBitCount = %d\n",Number,mBitCount);
      //mCallCount++;
      //// fprintf(stdout,"mCallCount: %d\n",mCallCount);
