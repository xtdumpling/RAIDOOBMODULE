  ...
  )
{
  ...
  if (gCopyRightMessage != NULL) {
    //
    // Intel Copyright message in Setup.
    //
    PrintStringAt (
      (gScreenDimensions.RightColumn + gScreenDimensions.LeftColumn - LibGetStringWidth (gCopyRightMessage) / 2) / 2,
  ...
  )
{
  ...
  gCopyRightMessage     = LibGetToken (STRING_TOKEN (COPYRIGHT_STRING), mCDLStringPackHandle);
