...
//
//  Rev. 1.00
//    Bug Fix : Reduced POST time when enabling FfsIntegrityCheck_SUPPORT and FFS_FILE_CHECKSUM_SUPPORT.
//              The POST time on PC 0x32 is reduced from 22 seconds to 13 seconds.
//              The POST time on PC 0x19 is reduced from 24 seconds to 1 second.
//              32 seconds are totally reduced during POST.
//    Reason  : Bug Fixed. (Refer to KabylakeDT Trunk revision #279.)
//    Auditor : Joe Jhang
//    Date    : Apr/25/2017
//
//****************************************************************************
  ...
  )
{
  ...
  UINT8                                 ErasePolarity;
  UINT8                                 FileState;
#if ((FfsIntegrityCheck_SUPPORT == 0) && (FFS_FILE_CHECKSUM_SUPPORT == 0))
  UINT8                                 DataCheckSum;
#endif  //#if ((FfsIntegrityCheck_SUPPORT == 0) && (FFS_FILE_CHECKSUM_SUPPORT == 0))
  BOOLEAN                               IsFfs3Fv;
