  ...
  )
{
  ...
  for (Index = 0; Index < 4; Index++) {
    ...
    while ((*Ip4Str != '\0') && (*Ip4Str != '.')) {
      //*** AMI PORTING BEGIN ***//
      // Bug fix: Original code does not properly validate the input string.
      // For example, string www.abc.com.cn is interpreted as a valid IP address.
      if ( (*Ip4Str < '0' || *Ip4Str > '9') && *Ip4Str != ' ' && *Ip4Str != '\t' ) {
        return EFI_INVALID_PARAMETER;
      }
      //*** AMI PORTING END *****//        
      Ip4Str++;
