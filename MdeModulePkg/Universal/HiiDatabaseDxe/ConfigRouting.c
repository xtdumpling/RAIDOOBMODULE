  ...
  while (*StringPtr != 0 && StrnCmp (StringPtr, L"GUID=", StrLen (L"GUID=")) == 0) {
    ...
    *(ConfigRequest + Length) = 0;
    ...
    if (IsEfiVarStore) {
      ...
      //*** AMI PORTING BEGIN ***//
      // Bug fix: The original code uses ConfigAccess without checking status of the previous gBS->HandleProtocol call.
      // We are also commenting out the ASSERT because there are some legitimate cases when an instance of ConfigAccess
      // may be not available.
      // For example, when we are restoring previously saved configuration and one of the previously present devices
      // has been unplugged.
