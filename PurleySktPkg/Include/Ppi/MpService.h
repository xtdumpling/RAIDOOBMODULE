/** @file
  When installed, the MP Services Ppi produces a collection of services 
  that are needed for MP management.

  The MP Services Ppi provides a generalized way of performing following tasks:
    - Retrieving information of multi-processor environment and MP-related status of
      specific processors.
    - Dispatching user-provided function to APs.
    - Maintain MP-related processor status.

  The MP Services Ppi may be produced on any system with more than one logical
  processor.

  MP Services Ppi is hardware-independent. Most of the logic of this Ppi
...
#ifndef _MP_SERVICE_PPI_H_
#define _MP_SERVICE_PPI_H_

//
// BUGBUG: EFI_AP_PROCEDURE is defined in PiDxeCis.h
//
#include <Pi/PiDxeCis.h>
#include <Protocol/MpService.h>

///
/// Global ID for the PEI_MP_SERVICES_PPI.
///
#define PEI_MP_SERVICES_PPI_GUID \
  { \
  0xee16160a, 0xe8be, 0x47a6, {0x82, 0xa, 0xc6, 0x90, 0xd, 0xb0, 0x25, 0xa} \
  }

///
/// Forward declaration for the PEI_MP_SERVICES_PPI.
///
typedef struct _PEI_MP_SERVICES_PPI PEI_MP_SERVICES_PPI;
...
/// that are needed for MP management.
///
/// Before the PI event END_OF_PEI is signaled, the module 
...
/// an AP before the PI event END_OF_PEI is signaled is 
/// implementation dependent.
///
/// After the PI event END_OF_PEI is signaled, all the APs 
...
/// END_OF_PEI to force APs into the OS compatible state as 
/// defined by the UEFI Specification.
///
/// The support for SwitchBSP() and EnableDisableAP() may no longer be supported 
/// after the PI event END_OF_PEI is signaled.
/// 
struct _PEI_MP_SERVICES_PPI {
  PEI_MP_SERVICES_GET_NUMBER_OF_PROCESSORS  GetNumberOfProcessors;
  PEI_MP_SERVICES_GET_PROCESSOR_INFO        GetProcessorInfo;
  PEI_MP_SERVICES_STARTUP_ALL_APS           StartupAllAPs;
  PEI_MP_SERVICES_STARTUP_THIS_AP           StartupThisAP;
  PEI_MP_SERVICES_SWITCH_BSP                SwitchBSP;
  PEI_MP_SERVICES_ENABLEDISABLEAP           EnableDisableAP;
  PEI_MP_SERVICES_WHOAMI                    WhoAmI;
};
