...
ApCFunction (
  ...
  IN UINTN                     ApIndex  //OVERIDE_HSD_5370234
  )
{
...
//OVERIDE_HSD_5370234_BEGIN
    BistData = *(UINTN *) (PeiCpuMpData->Buffer + ApIndex * PeiCpuMpData->CpuApStackSize - sizeof (UINTN));
    PeiCpuMpData->CpuData[ApIndex].ApicId        = GetInitialApicId ();
    PeiCpuMpData->CpuData[ApIndex].Health.Uint32 = (UINT32) BistData;
...
//OVERIDE_HSD_5370234_BEGIN
  //
  // AP finished executing C code, increment finished count and decrement
  // executing count
  //
  InterlockedIncrement ((UINT32 *)&PeiCpuMpData->FinishedCount);
  InterlockedDecrement ((UINT32 *)&PeiCpuMpData->MpCpuExchangeInfo->NumApsExecuting);
//OVERIDE_HSD_5370234_END
  ...
  )
{
  ...
  ExchangeInfo->ApIndex            = 0;  //OVERIDE_HSD_5370234
  ...
  )
{
  ...
  //
  // Wait for AP task to complete then exit.
  //
  MicroSecondDelay (PcdGet32 (PcdCpuApInitTimeOutInMicroSeconds));
//OVERIDE_HSD_5370234_BEGIN
  while (PeiCpuMpData->MpCpuExchangeInfo->NumApsExecuting != 0){
    CpuPause();
  };
//OVERIDE_HSD_5370234_END

  PeiCpuMpData->InitFlag  = 0;
  PeiCpuMpData->CpuCount += (UINT32) PeiCpuMpData->MpCpuExchangeInfo->ApIndex;  //OVERIDE_HSD_5370234
...
// APTIOV_SERVER_OVERRIDE_RC_START : Changes done to load Microcode from FV  
  ...
  MicrocodeEntryPoint = GetMicrocode((EFI_FIRMWARE_VOLUME_HEADER *)(UINTN)FV_MICROCODE_UPDATE_BASE, &UcSize);
    if (MicrocodeEntryPoint == NULL)
      MicrocodeEntryPoint = GetMicrocode((EFI_FIRMWARE_VOLUME_HEADER *)(UINTN)FV_MICROCODE_BASE, &UcSize);
#else      
  MicrocodeEntryPoint = GetMicrocode((EFI_FIRMWARE_VOLUME_HEADER *)(UINTN)FV_MICROCODE_BASE, &UcSize);
#endif
          
  PeiServicesAllocatePool ((UINTN) UcSize, (VOID **) &mMicrocodePatchAddress);
  CopyMem(mMicrocodePatchAddress, (VOID*)MicrocodeEntryPoint, UcSize);
// APTIOV_SERVER_OVERRIDE_RC_END : Changes done to load Microcode from FV
  //
