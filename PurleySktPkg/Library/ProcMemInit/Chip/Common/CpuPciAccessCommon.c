...
STATIC UINT8  FunTbl[MAX_CPU_TYPES][MAX_BOX_TYPES][8] = {
  {
    ...
    {0,    0,    7,    7,    4,    4,    0xFF, 0xFF}, // IIO_DFX             18
  ...
  )
{
  ...
  if (host != NULL) {
    for (socket = 0; socket < MAX_SOCKET; socket++) {
      CpuCsrAccessVar->stackPresentBitmap[socket] = host->var.common.stackPresentBitmap[socket];

      for (ctr = 0; ctr < MAX_IIO_STACK; ctr++) {
        CpuCsrAccessVar->StackBus[socket][ctr] = host->var.common.StackBus[socket][ctr];
      }

    CpuCsrAccessVar->SocketFirstBus[socket] = host->var.common.SocketFirstBus[socket];
    CpuCsrAccessVar->SocketLastBus[socket] = host->var.common.SocketLastBus[socket];
    CpuCsrAccessVar->segmentSocket[socket] = host->var.common.segmentSocket[socket];

    for (mc = 0; mc < MAX_IMC; mc++) {
        CpuCsrAccessVar->imcEnabled[socket][mc] = host->var.mem.socket[socket].imcEnabled[mc];
    }
    for (ch = 0; ch < MAX_CH; ch++) {
        CpuCsrAccessVar->mcId[socket][ch] = host->var.mem.socket[socket].channelList[ch].mcId;
      }
    }


    CpuCsrAccessVar->cpuType = host->var.common.cpuType;
    CpuCsrAccessVar->stepping = host->var.common.stepping;
    CpuCsrAccessVar->socketPresentBitMap = host->var.common.socketPresentBitMap;
    CpuCsrAccessVar->FpgaPresentBitMap = host->var.common.FpgaPresentBitMap;
    CpuCsrAccessVar->mmCfgBase = host->var.common.mmCfgBase;
    CpuCsrAccessVar->numChPerMC = host->var.mem.numChPerMC;
    CpuCsrAccessVar->maxCh = MAX_CH;
    CpuCsrAccessVar->maxIMC = host->var.mem.maxIMC;
  }
  ...
  )
{
  ...
  */
  ...
  case BOX_IIO_DFX:
    if ((CpuType == CPU_SKX) && (BoxInst < MAX_ALL_IIO_DFX)) {
      Dev = 7;
    } else {
