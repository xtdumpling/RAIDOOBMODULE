...
)
{
    ...
    //
    // Update CPU information on Main Setup Page from BSP revision using same method from SocketSetup.c
    //   
    Status = gBS->LocateProtocol (&gEfiIioUdsProtocolGuid, NULL, &IioUds);
    ASSERT_EFI_ERROR (Status);
    if ((IioUds->IioUdsPtr->SystemStatus.cpuType == CPU_SKX)) {
    
        switch (IioUds->IioUdsPtr->SystemStatus.MinimumCpuStepping) {
          case A0_REV_SKX:
              InitString(
                                   HiiHandle,
