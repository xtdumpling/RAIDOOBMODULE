...
//  IfrDeviceNvData = AllocateZeroPool (NIC_ITEM_CONFIG_SIZE);  // AMI PORTING : (EIP82809-)
    ...
    if (NicConfig == NULL) {
      ...
      FreePool (IfrDeviceNvData);  // AMI Porting - Cpp Check Memory Leak
...
/*AMI PORTING START : (EIP82809)
  ...
  if (!EFI_ERROR (Status)) {
    OldMenuString = NetLibHiiGetString (Instance->RegisteredHandle, STRING_TOKEN (STR_IP4_CONFIG_FORM_HELP), NULL);
