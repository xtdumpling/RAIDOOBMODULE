  ...
  )
{
  ...
  for (AipIndex = 0; AipIndex < AipHandleCount; AipIndex++) {
    ...
    if (((NetworkBootPolicy & STOP_UEFI_ISCSI_IF_AIP_SUPPORT_IP4) != 0 &&
      ...
      FreePool (InfoBlock);
      //APTIOV_SERVER_OVERRIDE_START: IScsiDriver should check pointer safety before using
      //
      // AMI PORTING STARTS
      //
      ...
      // AMI PORTING ENDS
      //
      //APTIOV_SERVER_OVERRIDE_END: IScsiDriver should check pointer safety before using
      continue;
    }
