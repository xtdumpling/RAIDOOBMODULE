...
//
//  Rev. 1.00
//    Bug Fix:  Add a AMI workaround to fix UEFI iSCSI malfunction for port 2
//              and port 1 with dedicated Lan installed.
//    Reason:   
//    Auditor:  Isaac Hsu
//    Date:     Feb/22/2017
//
...
#if SMCPKG_SUPPORT
  DEBUG((DEBUG_ERROR,"IScsiDoDhcp MediaPresent = %X\n", MediaPresent));
  if (!MediaPresent) {
      //
      // Give 100 milli seconds delay
      //
      gBS->Stall (ONE_SECOND * 10);
      MediaPresent = TRUE;
      Status = NetLibDetectMedia (Controller, &MediaPresent);
      DEBUG ((DEBUG_ERROR, "IScsi : IScsiDoDhcp Send again! MediaPresent=%x! Status = %r\n", MediaPresent, Status));
  }
#endif
  if (!MediaPresent) {
    return EFI_NO_MEDIA;
