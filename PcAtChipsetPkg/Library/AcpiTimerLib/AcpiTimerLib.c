  ...
  )
{
  ...
  //
// APTIOV_SERVER_OVERRIDE_RC_START : Following condition check is not needed as ACPI IO port is fixed build PCD
#if 0  
  if (PcdGet16 (PcdAcpiIoPciBarRegisterOffset) != 0x0000) {
...
#endif
// APTIOV_SERVER_OVERRIDE_RC_END : Following condition check is not needed as ACPI IO port is fixed build PCD
