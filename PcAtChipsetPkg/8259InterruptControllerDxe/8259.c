  ...
  )
{
  ...
  // APTIOV_SERVER_OVERRIDE_RC_START : Timer interrupt is masked after call to CSM 16 initialize yourself.
  // The value is written based upon 8259 PIC mode (Legacy/Protected). Protected Mode mask is 0xFFFE.
  // The SetMask API should also write to the PIC registers according.
  if (mMode == Efi8259ProtectedMode) {
  ...
  // APTIOV_SERVER_OVERRIDE_RC_END : Timer interrupt is masked after call to CSM 16 initialize yourself.
