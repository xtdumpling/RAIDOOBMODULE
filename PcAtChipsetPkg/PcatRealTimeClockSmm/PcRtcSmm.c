...
//APTIOV_SERVER_OVERRIDE_RC_START: Failed to get wakeup time with UEFI runtime service in Ubuntu Firmware Test Live
#define DATE_ALARM_MASK 0x3F 
//APTIOV_SERVER_OVERRIDE_RC_END: Failed to get wakeup time with UEFI runtime service in Ubuntu Firmware Test Live
...
// APTIOV_SERVER_OVERRIDE_RC_START
  ...
  //
  IoWrite8 (PCAT_RTC_INDEX_ALT, (UINT8) Address & PCAT_RTC_INDEX_MASK);
  return IoRead8 (PCAT_RTC_TARGET_ALT);
...
// APTIOV_SERVER_OVERRIDE_RC_START
  ...
  //
  IoWrite8 (PCAT_RTC_INDEX_ALT, (UINT8) Address & PCAT_RTC_INDEX_MASK);
  IoWrite8 (PCAT_RTC_TARGET_ALT, Data);
}
// APTIOV_SERVER_OVERRIDE_RC_END
...
// APTIOV_SERVER_OVERRIDE_RC_START: Acquire/Release Lock will raise TPL level, which might result in Assert because of TPL conflicts.
/*  
  //
...
*/
// APTIOV_SERVER_OVERRIDE_RC_END: Acquire/Release Lock will raise TPL level, which might result in Assert because of TPL conflicts.
  ...
  //
// APTIOV_SERVER_OVERRIDE_RC_START
  mModuleGlobal.CenturyRtcAddress = ACPI_CENTURY_CMOS;
// APTIOV_SERVER_OVERRIDE_RC_END
...
// APTIOV_SERVER_OVERRIDE_RC_END: Acquire/Release Lock will raise TPL level, which might result in Assert because of TPL conflicts.
  ...
  //
// APTIOV_SERVER_OVERRIDE_RC_START
  RegisterB.Data = RTC_INIT_REGISTER_B | (RegisterB.Data & (BIT5 | BIT2));
// APTIOV_SERVER_OVERRIDE_RC_END
...
// APTIOV_SERVER_OVERRIDE_RC_START : EIP269075, Improve uEFI Timer Services for Intel RC 82.D04.
    ...
    RtcWrite (ACPI_CENTURY_CMOS, DecimalToBcd8 ((UINT8) (RtcTime.Year / 100)));
  ...
  )
{
  ...
  EFI_TIME        RtcTime;
  UINTN           DataSize;
// APTIOV_SERVER_OVERRIDE_RC_START : EIP257030, Clarify the uEFI Timer Services Test fail on Purley.
  UINT8           Day;
// APTIOV_SERVER_OVERRIDE_RC_END : EIP257030, Clarify the uEFI Timer Services Test fail on Purley.
...
// APTIOV_SERVER_OVERRIDE_RC_END: Acquire/Release Lock will raise TPL level, which might result in Assert because of TPL conflicts.
  ...
  *Pending = RegisterC.Bits.Af;
  ...
  Time->Day    = RtcRead (RTC_ADDRESS_DAY_OF_THE_MONTH);
// APTIOV_SERVER_OVERRIDE_RC_START : EIP257030, Clarify the uEFI Timer Services Test fail on Purley.
  Day          = RtcRead (RTC_ADDRESS_REGISTER_D) & DATE_ALARM_MASK;  // APTIOV_SERVER_OVERRIDE_RC : Failed to get wakeup time with UEFI runtime service in Ubuntu Firmware Test Live.
  if (Day) {
    // RtcTimeFieldsValid() will the valid day later.
// APTIOV_SERVER_OVERRIDE_RC_START : Failed to get wakeup time with UEFI runtime service in Ubuntu Firmware Test Live.
    Day = BcdToDecimal8 (Day);
    if ((Day > 0) && (Day < 32))  {
      Day = DecimalToBcd8 (Day);
      Time->Day = Day;
    }
// APTIOV_SERVER_OVERRIDE_RC_END : Failed to get wakeup time with UEFI runtime service in Ubuntu Firmware Test Live.
  }
...
//    RtcTime.Day     = RtcRead (RTC_ADDRESS_DAY_OF_THE_MONTH);
    RtcTime.Day     = RtcRead (RTC_ADDRESS_REGISTER_D) & DATE_ALARM_MASK;  // APTIOV_SERVER_OVERRIDE_RC : Failed to get wakeup time with UEFI runtime service in Ubuntu Firmware Test Live.
// APTIOV_SERVER_OVERRIDE_RC_END : EIP257030, Clarify the uEFI Timer Services Test fail on Purley.
...
//    RtcWrite (RTC_ADDRESS_DAY_OF_THE_MONTH, RtcTime.Day);
    Day = RtcRead (RTC_ADDRESS_REGISTER_D) & (!DATE_ALARM_MASK);  // APTIOV_SERVER_OVERRIDE_RC : Failed to get wakeup time with UEFI runtime service in Ubuntu Firmware Test Live.
    Day |= RtcTime.Day;
    RtcWrite (RTC_ADDRESS_REGISTER_D, Day);
// APTIOV_SERVER_OVERRIDE_RC_END : EIP257030, Clarify the uEFI Timer Services Test fail on Purley.
...
//    Century++;
//  }
// EIP269075, Improve uEFI Timer Services for Intel RC 82.D04.
  if (mModuleGlobal.CenturyRtcAddress != 0) {
    Century = CheckAndConvertBcd8ToDecimal8 (RtcRead (mModuleGlobal.CenturyRtcAddress));
  } else {
